...
const MenuItemOption = ({ value, leftIcon, ...props }: IMenuItemOptionProps, ref: any) => {
	const { values, onChange, type }: IMenuOptionContextProps = useContext(MenuOptionContext);
	const isChecked = values.includes(value);
	const menuOptionProps = useMenuOptionItem({ isChecked, type });
	const { children, onPress, _icon, _text, ...resolvedProps } = usePropsResolution('MenuItem', props, { isChecked });
