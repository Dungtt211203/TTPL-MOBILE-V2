...
import type { IPopperProps, IScrollContentStyle, IArrowStyles, IPopoverArrowProps } from './types';
...
const PopperContent = React.forwardRef(({ children, style, ...rest }: IBoxProps & { isOpen: boolean }, ref: any) => {
	const {
		triggerRef,
		shouldFlip,
		crossOffset,
	...
	return (
		<View ref={overlayRef} collapsable={false} style={overlayStyle.overlay}>
			{arrowElement}
			<Box style={StyleSheet.flatten([containerStyle, style])} {...rest} ref={ref}>
				{restElements}
			</Box>
		</View>
	);
