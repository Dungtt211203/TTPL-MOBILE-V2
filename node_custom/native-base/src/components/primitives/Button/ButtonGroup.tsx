...
export default memo(
	forwardRef(({ children, divider, variant, ...props }: IButtonGroupProps, ref?: any) => {
		const {
			space = 2,
			direction = 'row',
			size,
		...
		if (Array.isArray(children)) {
			computedChildren = React.Children.toArray(children).map((child: any, index: number) => {
		...
		return (
			<Stack divider={divider} space={isAttached ? 0 : space} direction={direction} {...newProps} ref={ref}>
				{computedChildren}
			</Stack>
		);
	}),
