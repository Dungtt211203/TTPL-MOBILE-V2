...
const Checkbox = (
	...
	const { isInvalid, isReadOnly, isIndeterminate, ...combinedProps } = combineContextAndProps(
		formControlContext,
		props,
	);
...
const CheckboxComponent = React.memo(
	({
		...
		const [accessibilityProps, nonAccessibilityProps] = extractInObject(nonLayoutProps, [
			'accessibilityRole',
			'accessibilityState',
			'accessibilityLabel',
			'accessibilityHint',
		...
		return (
			...
			>
				<Stack {...layoutProps} {..._stack}>
					...
					{/* Label */}
					<Box flex={1}>{wrapStringChild(combinedProps.children, _text)}</Box>
				</Stack>
			</Pressable>
		);
	},
