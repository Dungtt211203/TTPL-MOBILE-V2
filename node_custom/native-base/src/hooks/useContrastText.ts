...
export function useContrastText(bg: string, color?: string) {
	const [contrastThreshold, trueDarkText, trueLightText, trueBg, trueColor] = useToken('colors', [
		'contrastThreshold',
		'darkText',
		'lightText',
		bg,
	...
	const suppressColorAccessibilityWarning =
		useNativeBaseConfig('NativeBaseConfigProvider').config.suppressColorAccessibilityWarning;
	...
	const textColor =
		!accessibleColors && bgThemeColorVariant && themeColorsThresholdShades[bgThemeColorVariant]
			? getContrastThemeColor(bgThemeColorVariant, bgShade)
			: getAccessibleContrastColor(
					typeof contrastThreshold === 'string' ? Number(contrastThreshold) : contrastThreshold,
					trueDarkText,
					trueLightText,
					trueBg,
					trueColor,
