...
import { propsFlattener, compareSpecificity, IStateProps } from './propsFlattener';
...
export const usePropsResolutionWithComponentTheme = (
	...
	const resolveResponsively = ['colorScheme', 'size', 'variant', ...(config?.resolveResponsively || [])];
	...
	const resolveComponentTheme = (themeType: Array<string>, providedTheme: any): any => {
		try {
			if (themeType[1]) {
	...
	const callPropsFlattener = (targetProps = {}, latestSpecifictyMap = {}, specificity = 1): any => {
	...
	extendedTheme.map((extededComponentTheme: any) => {
		...
		if (
			...
			if (
				typeof extededComponentTheme.sizes[incomingWithDefaultProps.size] === 'string' ||
				typeof extededComponentTheme.sizes[incomingWithDefaultProps.size] === 'number'
			) {
	...
	const resolvePseudoProps = (
		...
		for (const prop in flatPseudoProp) {
			baseStylePseudoProp[prop] =
				flatPseudoProp[prop]; /* Replace all the similar prop from from internal props */
		}
